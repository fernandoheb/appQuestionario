<?php
	...
	$query1 = $puxaBD->selectCustomQuery(
                'INSERT INTO `resposta`(`nome`, `email`, `genero`, `escolaridade`, `idade`) '
                . 'VALUES ( "'.utf8_decode($_POST["nomeApelido"]).'","'.utf8_decode($_POST["email"])
                .'","'.utf8_decode($_POST["generoSexual"]).'","'.utf8_decode($_POST["escolaridade"]).'","'.utf8_decode($_POST["idade"]).'")');
		...
		//$query1 = $puxaBD->selectCustomQuery('INSERT INTO `resposta`( `email`) VALUES ("'.utf8_decode($_POST["email"]).'")');	
               ...
               $email = utf8_decode($_POST["email"]);
		//echo consoleLog($email);
		$retorno = $puxaBD->selectCustomQuery("select * from resposta where email = '$email' order by id DESC");
